...
function App() {
  ...
  const associateNFTToken = async (id) => {
    ...
    const associateTransaction = new TokenAssociateTransaction()
      .setAccountId(customerAccount)
      .setTokenIds([id])
      .freezeWith(client);
    const signedAssociateTransaction = await associateTransaction.sign(
      customerKey
    );
    const transactionResponse = await signedAssociateTransaction.execute(
      client
    );
  ...
  const returnCar = async (id, serial) => {
    try {
      ...
      const allowanceApproveTxResponse =
        await new AccountAllowanceApproveTransaction()
          .approveTokenNftAllowanceAllSerials(
            id,
            customerAccount,
            smartContractAccountId
          )
          .freezeWith(client)
          .execute(client);
      client.setOperator(merchantId, merchantKey);
      console.log(
        `allowance approve tx response: ${allowanceApproveTxResponse}`
      );
